...
% current word count = 1200
    ...
    \paragraph{}
        ...
        \begin{itemize}
            \item explore the values of r and e, e.g. whether they are correlated across species (they aren't) (Figure \ref{fig:corrRE})
    ...
    \begin{figure}[H]
       % \hspace*{-3.5cm}
        \includegraphics[width=\textwidth]{../2.analyses/figures/pp_check0.png}
        \caption{Posterior predictive check showing the density distribution of observed seed production values (solid black line) to simulated seed production values (light grey). Simulated values were generated using the 80\% posterior confidence intervals for each parameter, the dark grey dashed line shows simulated values using the mean of each parameter. [UPDATE FIGURE]}
        \label{fig:ppcheck}
    \end{figure}
    ...
    % \newpage
        ...
        \caption{Interaction network between 22 focal species of wildflowers. (A) shows all observed and unrealised interactions whereas (B) shows those interactions estimated by the RIM only. Competitive interactions are shown in yellow and facilitative in blue, line thickness denotes interaction strength. Arrows point to species $i$. Interaction strengths here are given as per-capita values, averaged across all networks. [MAYBE MOVE FIG 5 D (proportion of asymetric interactions) HERE ??]}
    ...
    % IDK if it's worth keeping this figure - probably n ot if I don't want to mention co-occurrence methods? but maybe...
    ...
    \paragraph{}
    Potentially a short paragraph on 'network' results (weighted connectance, transitivity) so that I can later discuss how the scaling + network theory can allow us to link network patterns to diversity / stability etc. \\
    We calculated .... 
...
% word count aim: 500-1000
    ...
    \paragraph{}
    In ecology, interactions within a single trophic level are often difficult to observe, which means they are often quantified through other means \parencite{}. Unfortunately, these often require... 

    \paragraph{}
    Application of this framework to ecological setting: understanding diversity, importance of certain nodes etc. 

    \paragraph{}
    The application of this framework is not limited to ecological systems however, and can be used to 
    ...
    \paragraph{} % DELETE?
    The model framework we present here quantifies pair-wise interaction strengths between components from diverse horizontal community data. Several aspects of our model and its implementation allow us to better capture complex features of ecological communities compared to co-occurrence networks. Like co-occurrence networks, our framework is flexible to the inclusion of many species. Unlike co-occurrence networks however, interactions in our model are estimated from species effects on each other's performance. By couching our results into a model of population dynamics, we can go even further and capture how interactions may directly affect species abundances, rather than spatial association patterns. 

    % \paragraph{} % MOVE TO RESULTS?
