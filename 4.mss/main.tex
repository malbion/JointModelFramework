...
\paragraph{}
    ...
    Interaction effects are returned by the model as distributions capturing the likely effects of neighbours on the performance of focal components. Differences in the magnitude of these interaction terms may thus reflect differences in baseline performance (performance in the absence of neighbours, either competitive or facilitative) of the focal components. The interaction terms returned by both models above can be rescaled .... 
...
\textbf{Note: I've left the species / fitness nomenclature throughout and added a little sentence explaining why as I'm not sure that switching to components/performance will make it any clearer - happy to take advice on which makes more sense though}
    ...
    \subsection{Neighbour density-dependant model}
        ...
        \paragraph{}
        We begin by implementing a neighbour density-dependant model (NDDM) to each focal species $i$ which regresses the identity and abundance of neighbours $j$ ($j = 1, 2, 3, ...$) against the measured proxy for lifetime reproductive success $P_{i}$:
        
        \begin{equation}
        P_{i} = \beta_{i0} - \sum_{i}^{j} \beta_{ij} N_{j}
        \label{nddm}
        \end{equation}
    ...
    \subsection{Unrealised links}
        ...
        \begin{equation}
        P_{i} = \beta_{i0} - r_{i} \sum_{i}^{j} e_{j} N_{j}
        \label{rim}
        \end{equation}
    ...
    \subsection{Standardising effects into per-capita interactions}
        ...
        \begin{equation}
        P_{i} = \beta_{i0} \left ( 1 - \sum_{i}^{j} {\beta_{ij}}'' N_{j} \right )
        \label{LVform}
        \end{equation}
