...
\paragraph{}
    ...
    [THIS IS WHERE I BRING UP THE 'PROPER' SCALING - THOUGH THE NUMBERS PRESENTED PREVIOUSLY RELY ON THE pop dyn model alphas, I thought that bringing all of this up earlier wouldn't fit the results quite so well. FEEL FREE TO DISAGREE]

    \paragraph{}


    Additionally, one may be interested in other things than lifetime reproductive success ...

        Standardisation: I can talk about the scaling here - both the scaling to put them on the same scale, and to make predictions of dynamics. 

    And also to make predictions on other things like diversity... 

    Give estimates of effect sizes - eg effect of scaled alphas on growth as percentage ?


    \paragraph{} 
    % Case study specific results in the context of ecology 
    % - Firstly, intraspecific interactions ($\alpha_{ii}$) are estimated for all focal species and allow us to quantify how much a species regulates itself. In our case study, 85\% of all species across all network samples showed intraspecific competition.
...
\textbf{Note: I've left the species / fitness nomenclature throughout and added a little sentence explaining why as I'm not sure that switching to components/performance will make it any clearer - happy to take advice on which makes more sense though}
    ...
    \subsection{Neighbour density-dependant model}
        ...
        % This needs to be in an exponential form?! or is that only in the code??
