...
\section{Introduction}
    ...
    \paragraph{} 
    A common alternative which is less demanding and scalable to high-diversity systems is the use of association, or co-occurrence networks. Plant interactions are inferred by measuring positive and negative spatial associations between species (e.g. \citet{Saiz2011}), resulting in networks which capture a variety of factors including biotic interactions \citep{Losapio2019}. This approach requires data which is much more easily obtainable and can naturally handle a large number of species. A wide range of packages are available in R to deduce association networks, alongside further tools and methods to better infer plant interactions from spatial distribution patterns \citep{Keil2019}, for example by including environmental covariates. Spatial association networks however, caputre a wide range of other ecological processes (e.g. dispersal, habitat filtering) and have been found to remain poor predictors of species interactions,  rarely matching empirical estimates \citep{Sander2017,Barner2018, Thurman2019, Blanchet2020}.

    % Furthermore, this method is typically unable to resolve interactions between large numbers of species, because you need a lot of data (lots of replication) and complex models which run the danger of bering overparameterised amongst other things.

    %  This is often done a low diversity scales (few species) because we need a lot of replication! 
    ...
    \paragraph{}
    We propose a general framework to estimate interactions between plants (or other single-trophic systems) in natural and complex communities. Our method extends population dynamic approaches to quantifying interactions by allowing for facilitative interactions...
    To deal with diversity, our method merges two population models and couches our framework in bayesiam ... to ... 

    We allow for facilitative and competitive interactions both between and within species. This framework merges multiple approaches and is implemented with Bayesian statistics which allows us to estimate interactions between many species. 
