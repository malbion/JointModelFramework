...
\label{SI:Methods}
    ...
    \begin{figure}[H]
        ...
        \caption{Distribution of the split $\hat{R}$ statistic from the model run on our case study data with three chains of 4000 iterations each, with the first 2000 discarded. The graph shows that $\hat{R}$ is centred on 1 and does not exceed $1.01$, indicating that chains have converged to the same underlying distribution. This figure was plotted using the stan\_rhat() function from the rstan package \parencite{Rstan2020}.}
        \label{fig:adist}
    \end{figure}
