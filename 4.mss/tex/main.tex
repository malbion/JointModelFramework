...
\section{Introduction}
    ...
    \textbf{Various methods have been developed to circumventing these issues. A common strategy is to reduce the number of parameters to be estimated by assuming many interactions are weak enough to be negligeable, and thus do not need to be inferred \parencite{Weiss-Lehman2022}. Other approaches average interactions across species, grouping them for example by origin or life form \parencite{Martyn2021}, or by taxonomy and traits \parencite{Uriarte2004}. Alternatively, interactions are not identified and models instead rely on estimating spatial associations between species, which include the effects of interactions. [DELETE PREVIOUS SENTENCE? CO-OCCURRENCE METHODS ARE NOT BROUGHT UP AGAIN.] Here we present another alternative, a joint model which allows us to estimate both pairwise identifiable and pairwise unidentifiable interactions from measures of performance in the absence and presence of different interaction partners.}
...
%, i.e. when elements are observed to co-occur enough times and at varying densities such that the effect of one element on the performance of the other can be reliably estimated via regression. Conversely, the interaction effect of element $j$ on element $i$ is is deemed unidentifiable when there are none or too few measures of the performance of $i$ in the presence of varying densities of $j$. 
    ...
    % Last sentence moved from Discussion
...
\section{Discussion}
    ...
    Bayesian variable selection - e.g. Weiss-Lehman 2022. sparsity- priors to retain some interactions and replace others with an average term to reduce model parameter numbers. Aim of method is to reduce number of parameters to explore species interactions in diverse communities. 
    Define interactions as a combination of an average interaction and species-specific deviations from this average.
    reduce number of parameters by identifying only the species-specific deviation from the average. Allow a subset of species-specific parameters to take non-zero values
    Fit full model with regularised horseshoe priors to n fit final model using non-sparse methods.induce sparsity, the

    Picoche and Barraquand 2020 multivariate autoregressive model to temporally replicated data. 

    Ovaskainen et al 2017 which linear combinations of species abundances best predict future growth rates - groups species based on hteir competitive effects but doesn't accoutn for environmental variation
    time-series model
    rather than assume sparse interactions (Mutshinda 2009), assume that interations can be described by a small number of community-level drivers aka linear combinations of species occurrences. Assume there are far fewer drivers than species. Need all unique intra alphas
    They compare their approach to a sparse and full interaction model, full interaction performs badly due to overfitting
